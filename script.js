...
function getBounds() {
  const visualAdjust = 15; // tweak this slightly
 ...
 function playHoverHint() {
  if (hintPlayed || isUserDragging) return;

  hintPlayed = true;

  const bounds = getBounds();
  const startX = gsap.getProperty(squeegee, "x");
  const endX = bounds.maxX - 5;

  hintTween = gsap.timeline();

  // 👉 slide ALL the way right slowly
  hintTween.to(squeegee, {
    x: endX,
    duration: 1.8, // 👈 slower (increase if you want even slower)
    ease: "power1.inOut",
  ...
  frame.addEventListener("mouseenter", playHoverHint, { once: true });
